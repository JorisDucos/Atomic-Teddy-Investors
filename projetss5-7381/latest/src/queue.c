...
struct teddy teddy_init()
{
  for (int i = 0; i < MAX_PLYR; i = i + 1)
  {
    ...
    all_teddy[i].wallet.data[1] = 1000;
    all_teddy[i].wallet.data[2] = 1000;
    all_teddy[i].id = i;
    all_teddy[i].time = 0;
    all_teddy[i].localisation = starting_stockex();
    all_teddy[i].visited_stockex[0] = stockex__name(starting_stockex());
    all_teddy[i].len_visited_stockex=1;
  }
  return (*all_teddy);
}
